...
class Cell {
    constructor() {
        this.bits = new bitString_1.BitString(1023);
        this.refs = [];
        this.isExotic = false;
    }
    /**
    ...
    toBoc(has_idx = true, hash_crc32 = true, has_cache_bits = false, flags = 0) {
        ...
        const serialization = new bitString_1.BitString((1023 + 32 * 4 + 32 * 3) * topologicalOrder.length);
        ...
        if (hash_crc32) {
            ser_arr = (0, utils_1.concatBytes)(ser_arr, (0, utils_1.crc32c)(ser_arr));
        }
        return ser_arr;
    }
    ...
    serializeForBoc(cellsIndex, refSize) {
        ...
        for (let k in this.refs) {
            ...
            const reference = (0, utils_1.hexToBytes)(refIndexHex);
    ...
    beginParse() {
        ...
        return new slice_1.Slice(this.bits.array.slice(), this.bits.length, refs);
    }
}
exports.Cell = Cell;
function moveToTheEnd(indexHashmap, topologicalOrderArray, target) {
...
exports.moveToTheEnd = moveToTheEnd;
/**
 * @param cell  {Cell}
 ...
 * @return {[[], {}]} topologicalOrderArray and indexHashmap
 */
function treeWalk(cell, topologicalOrderArray, indexHashmap, parentHash = null) {
    ...
    for (let subCell of cell.refs) {
        const res = treeWalk(subCell, topologicalOrderArray, indexHashmap, cellHash);
...
function parseBocHeader(serializedBoc) {
    ...
    if ((0, utils_1.compareBytes)(prefix, reachBocMagicPrefix)) {
    ...
    if ((0, utils_1.compareBytes)(prefix, leanBocMagicPrefix)) {
        has_idx = 1;
        hash_crc32 = 0;
        has_cache_bits = 0;
        flags = 0;
    ...
    if ((0, utils_1.compareBytes)(prefix, leanBocMagicPrefixCRC)) {
        has_idx = 1;
        hash_crc32 = 1;
        has_cache_bits = 0;
        flags = 0;
    ...
    serializedBoc = serializedBoc.slice(1);
    const cells_num = (0, utils_1.readNBytesUIntFromArray)(size_bytes, serializedBoc);
    serializedBoc = serializedBoc.slice(size_bytes);
    const roots_num = (0, utils_1.readNBytesUIntFromArray)(size_bytes, serializedBoc);
    serializedBoc = serializedBoc.slice(size_bytes);
    const absent_num = (0, utils_1.readNBytesUIntFromArray)(size_bytes, serializedBoc);
    serializedBoc = serializedBoc.slice(size_bytes);
    const tot_cells_size = (0, utils_1.readNBytesUIntFromArray)(offset_bytes, serializedBoc);
    ...
    for (let c = 0; c < roots_num; c++) {
        root_list.push((0, utils_1.readNBytesUIntFromArray)(size_bytes, serializedBoc));
        serializedBoc = serializedBoc.slice(size_bytes);
    }
    let index = false;
    if (has_idx) {
        ...
        for (let c = 0; c < cells_num; c++) {
            index.push((0, utils_1.readNBytesUIntFromArray)(offset_bytes, serializedBoc));
            serializedBoc = serializedBoc.slice(offset_bytes);
        }
    }
    ...
    if (hash_crc32) {
        ...
        if (!(0, utils_1.compareBytes)((0, utils_1.crc32c)(inputData.slice(0, length - 4)), serializedBoc.slice(0, 4)))
...
function deserializeCellData(cellData, referenceIndexSize) {
    ...
    for (let r = 0; r < refNum; r++) {
        cell.refs.push((0, utils_1.readNBytesUIntFromArray)(referenceIndexSize, cellData));
...
function deserializeBoc(serializedBoc) {
    if (typeof serializedBoc == "string") {
        serializedBoc = (0, utils_1.hexToBytes)(serializedBoc);
    }
