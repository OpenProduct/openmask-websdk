...
class WalletV4ContractR2 extends walletContract_1.WalletContract {
    ...
    deployAndInstallPlugin = (params) => contract_1.Contract.createMethod(this.provider, this.deployAndInstallPluginImp(params));
    installPlugin = (params) => contract_1.Contract.createMethod(this.provider, this.installPluginImp(params));
    getName() {
        return "v4R2";
    }
    /**
     ...
     */
    async deployAndInstallPluginImp(params) {
    ...
    async _setPlugin(params, isInstall) {
        ...
        const pluginAddress = new address_1.default(params.pluginAddress);
        const signingMessage = this.createSigningMessage(seqno, true);
        ...
        signingMessage.bits.writeGrams(amount || (0, utils_1.toNano)("0.1"));
        signingMessage.bits.writeUint(queryId || 0, 64);
        return this.createExternalMessage(signingMessage, secretKey, seqno, false);
    }
    async installPluginImp(params) {
     ...
     */
    isPluginInstalled = async (pluginAddress) => {
        pluginAddress = new address_1.default(pluginAddress);
        const hashPart = "0x" + (0, utils_1.bytesToHex)(pluginAddress.hashPart);
        const myAddress = await this.getAddress();
