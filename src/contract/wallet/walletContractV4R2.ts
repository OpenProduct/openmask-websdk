...
export class WalletV4ContractR2 extends WalletContract {
  ...
  constructor(provider: HttpProvider, options: Options) {
    ...
    this.methods.deployAndInstallPlugin = (params: DeployAndInstallPlugin) =>
      Contract.createMethod(provider, this.deployAndInstallPlugin(params));
    this.methods.installPlugin = (params: PluginParams) =>
      Contract.createMethod(provider, this.installPlugin(params));
    this.methods.removePlugin = (params: PluginParams) =>
      Contract.createMethod(provider, this.removePlugin(params));
   ...
   * @return {Cell}
   */
  createSigningMessage(seqno?: number, withoutOp?: boolean) {
    seqno = seqno || 0;
   ...
   */
  async deployAndInstallPlugin(params: DeployAndInstallPlugin) {
   ...
   */
  async _setPlugin(params: PluginParams, isInstall: boolean) {
   ...
   * @param   params {{secretKey: Uint8Array, seqno: number, pluginAddress: string | Address, amount?: BN, queryId?: number}}
   */
  async installPlugin(params: PluginParams) {
   ...
   */
  async isPluginInstalled(pluginAddress: string | Address) {
