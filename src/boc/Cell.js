...
export class Cell {
  ...
  async toBoc(
    ...
    const serialization = new BitString(
      (1023 + 32 * 4 + 32 * 3) * topologicalOrder.length
    );
    ...
    if (has_idx) {
      topologicalOrder.forEach((cell_data, index) =>
...
export function parseBocHeader(serializedBoc) {
  ...
  if (hash_crc32) {
    ...
    if (
      !compareBytes(
        crc32c(inputData.slice(0, length - 4)),
        serializedBoc.slice(0, 4)
      )
    )
